<template>
  ...
  const searchQuery = computed({
    get: () => store.state.posCash.searchQuery,
    set: (value) => store.commit('posCash/setSearchQuery', value),
  });
  
  const expandedRows = ref([]);
  const filteredArr = computed(() => store.getters['posCash/filteredArr']);
  ...
  const filterData = () => {
    store.commit('posCash/setSearchQuery', searchQuery.value);
  };
  </script>
